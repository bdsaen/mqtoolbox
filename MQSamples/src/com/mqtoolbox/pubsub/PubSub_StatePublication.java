 ...
 * example, the current temperature. As this doesn't use a retained publications, the subscriber will only get
 * publications put after they subscribed. Any publications put before they subscribed are not available.
 */
public class PubSub_StatePublication {
	...
	public static void main(String[] args) {
		PubSub_StatePublication pvt = new PubSub_StatePublication();
...
//			statePublication_CurrentTemperature.setIntProperty(WMQConstants.JMS_IBM_RETAIN, WMQConstants.RETAIN_PUBLICATION);
			...
			// Now get the latest publication. As the publication was not set as retained, there are no publications to consume.
			...
			// Put a publication. In this case there is a subscriber. As the publication stays until all subscribers consume the
			// publication, the publication will be found.
			statePublication_CurrentTemperature = pvt.getSession().createTextMessage("33C");
//			statePublication_CurrentTemperature.setIntProperty(WMQConstants.JMS_IBM_RETAIN, WMQConstants.RETAIN_PUBLICATION);
			publisher.send(statePublication_CurrentTemperature);

			// Now get the latest publication. As the subscriber was created before the publication was put, it will get a publication.
			msg = subscriber.receive(5000); // milliseconds
			if (msg == null)
				System.out.println("No publication");
			else
